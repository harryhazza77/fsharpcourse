...
// ------------------------------------------------------------------
// TASKS: Fill in the holes in the following tests
// ------------------------------------------------------------------

// TASK #1: Create a test that checks whether 
// 'Analytics.countProducts' work on the following
// sample list of products:
//
//  let products = 
//    [ Product("Tea", 1.0M<GBP>)
//      Product("Newspaper", 2.0<GBP>)
//
  
let [<Test>] ``Non-empty operator name is the name of the operator``() =
  Check.Quick(fun (name:string) -> 
    // TASK #2: Check that calling 'Analytics.operatorName' on 
    // an option value that contains the randomly provided name
    // (created using the 'Some' constructor)
    failwith "TODO" 
  )
...
do
  ...
  ``Some "Tomas" returns "Tomas" as the operator`` ()
